...
use crate::cli::Cli;
use crate::metrics::{Device, ExportableMetric, Metrics, set_jellyfin_up};
...
pub async fn handle_request(cli: &Cli, client: &Client, metrics: &mut Metrics) -> Result<(), reqwest::Error> {
    ...
    if let Ok(users) = log_error!(users, "Could not get Users") {
        ...
        for (user, items) in items {
            ...
            for item in &items {
                item.set_metrics(metrics, &user);
            }
        }
        debug!("Done exporting metrics in {:?}", Instant::now() - s)
    };
